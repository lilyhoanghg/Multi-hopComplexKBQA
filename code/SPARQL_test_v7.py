...
def SQL_1hop_reverse(p, const_entities, QUERY=None):
    '''
    Modified for METAQA2
    '''
    kbs, const1, sparql_txts = defaultdict(set), '', set()
    # raw_trips, t_idx, d_idx = form_trips(p)
    raw_trips, t_idx, d_idx = form_trips_metaqa(p)
    # if re.search('^[mg]\.', list(const_entities)[0]):
    if "movie" in list(const_entities)[0] or "entity" in list(const_entities)[0]:
        const_type = 'mid'
        # const_entiti = ['ns:%s' %e for e in const_entities]
        const_entiti = const_entities
    ...
    for q_idx, query in enumerate(queries):
        ...
        sparql_txt = """%s\nSELECT DISTINCT %s WHERE {%s\n%s}%s""" % (
        PREFIX, retu, trips, const, const1)
        # print(sparql_txt)
        # print(QUERY)
        sparql = SPARQLWrapper(SPARQLPATH)
        sparql.setQuery(sparql_txt)
        ...
        try:
            ...
            if results['results']['bindings']:
                for t in results['results']['bindings']:
                    ...
                    # r = t['r']['value'].split('/ns/')[-1] if re.search('^http', t['r']['value']) else t['r']['value']
                    # t = t['e%s' %(t_idx)]['value'].split('/ns/')[-1] if re.search('^http', t['e%s' %(t_idx)]['value']) else t['e%s' %(t_idx)]['value']
                    h = "{}:{}".format(SHORT_PREFIX, t['e%s' % (t_idx + 1)]['value'].split('#')[-1]) if re.search('^http', t['e%s' % (t_idx + 1)]['value']) else t['e%s' % (t_idx + 1)]['value']
                    r = "{}:{}".format(SHORT_PREFIX, t['r']['value'].split('#')[-1]) if re.search('^http', t['r']['value']) else t['r']['value']
                    t = "{}:{}".format(SHORT_PREFIX, t['e%s' % (t_idx)]['value'].split('#')[-1]) if re.search('^http', t['e%s' % (t_idx)]['value']) else t['e%s' % (t_idx)]['value']
