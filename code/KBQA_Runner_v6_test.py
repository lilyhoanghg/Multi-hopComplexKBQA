...
def create_instances(input_file, tokenizer, verbose = False):
    ...
    for q_idx, q in enumerate(questions):
        # if verbose:
        #     print("raw_question: {}".format(raw_questions[q_idx]))
        #     print("question: {}".format(questions[q_idx]))
        #     print("topic_entity: {}".format(topic_entities[q_idx]))
        #     print("constraint: {}".format(constraints[q_idx]))
        #     print("answer: {}".format(answers[q_idx]))
        #     print("golden_graph: {}".format(golden_graphs[q_idx]))
...
def retrieve_KB(batch, KB, QUERY, M2N, tokenizer, method, train_limit_number=1000, time = 0, is_train = False, save_model='SO',
    ...
    for previous_path in set(c_te.values()):
        ...
        if previous_path in KB:
            ...
            if verbose:
                print("[retrieve_KB] previous_path in KB, paths: {}".format(paths))
                # print("[retrieve_KB] len KB: {}".format(len(KB)))
                # if len(KB) < 2: print("[retrieve_KB] KB: {}".format(KB))
        # MetaQA test
        # path, query = SQL_1hop(previous_path, QUERY=QUERY)
        # if verbose: print("path, query: {}, {}".format(path, query))
        # input()

        # if time: # commented on 30 nov 2022.
        # if tokenizer.dataset in ['CWQ'] and time < 2: # commented on 19 Jan 2023
        # #(tokenizer.dataset in ['CWQ'] and time < 2) or (len(paths) == 0 and time==0):
        # #(tokenizer.dataset in ['CWQ'] and time < 2) or
        ...
        if time:
            ...
            const = set(te.keys()) - overlap_te
            if verbose: print(te); print(const); print(batch.const_time); print(batch.const_minimax); print(batch.const_type)
            const_list = list(const)
            for i, item in enumerate(const_list):
                item_name = strip_wikimovies_prefix(const_list[i], SHORT_PREFIX)
                const_list[i] = item_name
            const = set(const_list)
    ...
    for p_idx, p in enumerate(raw_candidate_paths):
        ...
        if verbose: print("[retrieve_KB] CWQ_F1: {}".format(CWQ_F1))
        if (not is_train) or (np.random.random() < (limit_number*1./len(raw_candidate_paths))) or CWQ_F1 > 0.5:
            ...
            for w in sum(p, ()):
                ...
                if not re.search('^\?', w):
                    ...
                    elif len(w.split('_')) > 1: # When it's a relation metaqa 2.0
                        path += w # TODO: check this expression. refer to above.
            ...
            if verbose:
                print("[retrieve_KB] answer: {}".format(answer))
...
def main():
    ...
    parser.add_argument("--do_eval", default=1, type=int, help= "Whether to run eval. if do_eval is 2, cached data is not used.")
    ...
    parser.add_argument("--verbose", default=False, type=bool, help="whether to print parameters, for debugging")
    ...
    if verbose: print("max_hop_num: {}".format(args.max_hop_num))
    ...
    if tokenizer.dataset not in ["CWQ", "CQ", "WBQ"]: # to default to CWQ actions.
    ...
    # if verbose:
    #     print("First KB key: {}".format(list(KB.keys())[0]))
    #     print("First KB value: {}".format(list(KB.values())[0]))
            ...
            if eval_reward >= max_eval_reward:
                ...
                for eval_step, batch in enumerate(test_instances): #[328:329]
                    ...
                    while time < args.max_hop_num:
                        ...
                        query_num += qr_n
                        if verbose: print(cp, ts, tn, ty_n, su_n, ye_n, an_n, hn, RAs, mcl, qr_n, done)

                        if len(cp) == 0:
                            skip_forward = True
                            break
                        ready_batch = select_field(batch.question, cp, ts, tn, ty_n, su_n, ye_n, an_n, hn, RAs, mcl, method=config.method)
                        # if verbose: print(ready_batch)
                        if check_args_gpu_id: ready_batch = tuple(t.to(device) for t in ready_batch)
                        ...
                        # if verbose: print(logits, action, batch)
                        # if verbose: print(reward, pred_cp, done, pred_ans, top_pred_ans)
                        update_train_instance(batch, action)
                        if done: break
                        time += 1
